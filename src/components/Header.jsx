...
export function Header({ navItems, profileImage, resumeUrl }) {
  // Local state for toggling the mobile menu visibility on small screens
  const [isOpen, setIsOpen] = useState(false)
  const { theme, isDark, toggleTheme } = useTheme()

  // Event listener to automatically close the mobile menu when a navigation item 
  // is clicked (detected via browser hash change). This ensures the menu 
  // isn't left hanging open after a jump to a page anchor.
  useEffect(() => {
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
        ...
        <nav className="hidden items-center gap-8 lg:flex" aria-label="Primary">
          ...
          >
            {isDark ? (
              // Sun icon (appears in Dark mode to switch back to Light)
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-5 w-5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v2.25m6.364.386-1.591 1.591M21 12h-2.25m-.386 6.364-1.591-1.591M12 18.75V21m-4.773-4.227-1.591 1.591M5.25 12H3m4.227-4.773L5.636 5.636M15.75 12a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0Z" />
              </svg>
            ) : (
              // Moon icon (appears in Light mode to switch back to Dark)
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-5 w-5">
      ...
      {/* Mobile Menu Panel: Rendered conditionally based on state */}
      {isOpen ? (
        <nav
          id="mobile-nav"
          aria-label="Mobile"
